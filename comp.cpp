...
class computer{
public:
    string type[50],processor[50];
    int ram[50],usb[50],hdmi[50],hardisk[50],battery[50];
};
class person{
public:
...
if(y==1){
    ...
    cout<<"battery backup of the device:"<<endl;
    cin>>s3.battery[i];
    while (cin.fail()||s3.battery[i]<=0)
    {
        cin.clear(); // clear input buffer to restore cin to a usable state
        cin.ignore(INT_MAX, '\n'); // ignore last input
        cout << "invalid input\n";
    ...
    file<<"battery backup of the device:"<<s3.battery[i]<<endl;
...
for (int i = 0; i < n1; i++)
{
    ...
    my<<"battery backup of the device:"<<s3.battery[i]<<endl;
    my<<endl;   
}
}
if(y==2){
...
if(k==(i+1))
{
    ...
    case 5:
    int x;
    cout<<"What Specific do you want to change?:";
    cout<<"(1)Processor\n(2)RAM\n(3)Hard-disk\n(4)USB\n(5)HDMI";
    cin>>x;
    cin.ignore();
    switch(x){
...
me<<"processor:"<<s3.processor[i]<<endl;
...
me<<"battery backup time in hrs:"<<s3.battery[i]<<endl;
me<<endl;;
}}
if (n1==0)
{ 
...
bsl<<"processor:"<<s3.processor[i]<<endl;
...
bsl<<"battery backup time in hrs:"<<s3.battery[i]<<endl;
bsl<<endl;
}}
if (n1==0)
{
